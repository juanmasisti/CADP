...
procedure almacenarVenta (var v:vector; dml:rango3);
var	
	p:venta;
begin
	dml:=0;
	leerVenta(p);
	while(dml<dimf) and (p.codigoP <> 0) do begin
		dml:=dml+ 1;
		v[dml]:=p;
		leerVenta(p);
	end;
...
var posInferior, posSuperior, salto, i, j: rango3; 
Begin
